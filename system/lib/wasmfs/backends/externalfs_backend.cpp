...
private:
  std::shared_ptr<File> getChild(const std::string& name) override {
    ...
    if (childID == -1) {
      ...
      if (childID == -EACCES)
        fprintf(stderr, "File %s requires read permission\n", name.c_str());
      return NULL;
    }
    if (childType == 1) {
...
public:
  ...
  ExternalFSBackend(const std::string& directory_name): directory_name(directory_name) {}
  ...
  std::shared_ptr<Directory> createDirectory(mode_t mode) override {
    int success = 0;
    proxy([&](auto ctx) {
      _wasmfs_externalfs_init_root_directory(ctx.ctx, directory_name.c_str(), &success);
    });

    return success ? std::make_shared<ExternalFSDirectory>(mode, this, 1, proxy) : nullptr;
  }
...
backend_t wasmfs_create_externalfs_backend(char const *directory_name) {
  ...
  assert(
    !emscripten_is_main_browser_thread() &&
      "Cannot safely create ExternalFS backend on main browser thread");
  return wasmFS.addBackend(std::make_unique<ExternalFSBackend>(directory_name));
}
